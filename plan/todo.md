...
**Depends on:** TASK-P1-IAM-002
**Files (create/modify):**
  - `services/iam/internal/oidc/discovery.go` (new) — `/.well-known/openid-configuration`
  - `services/iam/internal/oauth2/authorize.go` (new) — `/oauth2/authorize`; PKCE S256 mandatory
  - `services/iam/internal/oauth2/token.go` (new) — `/oauth2/token` (auth-code, refresh, client-credentials)
  - `services/iam/internal/oauth2/userinfo.go` (new) — `/oauth2/userinfo`
  - `services/iam/migrations/0005_oauth2_clients.sql` (new) — `oauth2_clients` (client_id, client_secret_hash, redirect_uris[], grant_types[], scopes[])
  - `services/iam/test/oidc_e2e_test.go` (new) — end-to-end auth-code flow with PKCE
**Acceptance criteria:**
  1. PKCE S256 challenge required; missing/plain challenge rejected.
  2. Discovery doc validates against the OpenID Connect Discovery 1.0 spec.
  3. Client-credentials grant supports machine-to-machine flows for internal service tokens.
**Verification:**
  - Integration: `services/iam/test/oidc_e2e_test.go` (uses `coreos/go-oidc/v3` as a conformance client).
