...
**Files (create/modify):**
  - `bench/k6/iam-login.bench.js` (new) — k6 ramping-arrival-rate scenario pinned at 1 000 req/s for 5 min (ramp 30s → hold 4m30s → ramp 30s; ~270 000 samples — statistically tight p95). Workload mix: 60 % login-ok (password + TOTP) / 20 % WebAuthn assertion / 15 % refresh-token rotation / 5 % bad-credentials. Pre-seeded user pool of ≥1 000 accounts so the per-user rate-limiter (TASK-P1-IAM-001) doesn't skew p95. Refresh tokens captured from the success branch feed the rotation branch (single-use per chetana contract — bench replaces the consumed token in-place). **Per-tag thresholds**: `login-ok` / `webauthn` / `refresh` p95 < 100 ms; `login-bad` 200 ms ≤ p95 < 350 ms — the lower bound enforces the constant-time floor (REQ-FUNC-PLT-IAM-010); the upper bound catches regressions where a wrong-password short-circuits the floor (would re-open the user-enumeration channel). `http_req_failed{tag:login-ok}` < 0.1 %.
  - `bench/k6/realtime-fanout.bench.js` (new) — 10 000 concurrent WebSocket connections (`ramping-vus`: ramp 60s → hold 2 min → ramp 30s). 100 test topics (`telemetry.bench.{0..99}`) with 100 subscribers each. The bench's `setup()` triggers the chetana producer harness via HTTP control endpoint (`/bench/producer/start`) which fires 10 events/sec on each of 100 topics for 200 seconds — 100 events/sec × 100 topics × 100 subscribers = 1 000 000 push deliveries per second steady-state. **Push latency** = `Date.now() - server_emitted_at_ms` (server-stamped epoch ms in every payload) — covers producer → Kafka → realtime-gw Redis fan-out → backpressure buffer → WebSocket frame end-to-end. Threshold `chetana_rt_push_latency_ms p(95) < 500`. Bearer is supplied via the chetana sub-protocol channel (`Sec-WebSocket-Protocol: chetana.v1, chetana.bearer.<token>`) matching the production WS upgrade contract. Session-establishment failure threshold `count<100` (≤ 0.1 % of 100 000 connect attempts).
  - `bench/Taskfile.yml` (modify) — added `bench:iam`, `bench:realtime`, `bench:phase1` (runs both in sequence), `bench:report:phase1` (one-line p95 verdict per bench from the summary JSON via dependency-free `grep -oE '"p\(95\)":[0-9.]+'` so the GHA k6 image doesn't need python). Each task writes `bench/results/phase1/<name>.summary.json` (k6 `--summary-export`) + `<name>.run.json` (full sample stream) so the workflow can archive both as artifacts.
  - `.github/workflows/nfr-phase1.yml` (new) — three-stage workflow: `bring-up` (provisions the ephemeral cluster — currently a placeholder that fails loudly when no cluster URL is supplied via `vars.NFR_PHASE1_*` repo variables; the real provisioner lands with TASK-P1-CLUSTER-001) → `iam-bench` + `realtime-bench` (each a 2-element matrix `{run: [1, 2]}` with `fail-fast: true` so the second run is skipped on first-run failure — implements acceptance #1's "two consecutive runs" requirement) → `gate` (aggregator that fails when EITHER bench result is non-`success` OR the bring-up was skipped). Triggers on PRs touching `services/iam/**`, `services/packages/{authz,audit}/**`, `services/realtime-gw/**`, `bench/**`, OR pushes to `main`. Per-job timeouts: 25 min IAM (5 min bench × 2 + setup), 30 min realtime (3 min bench × 2 + setup + ulimit). The realtime job lifts `ulimit -n` to 131 072 so 10 k WS connections don't silently exhaust the runner's default 65 535 fd cap. **Acceptance #2 wiring**: the `gate` job is the required status check on the `main` branch protection rule (configured via repo settings, not in this file); k6 thresholds inside each script fail the bench → fails the matrix entry → fails the gate → blocks merge.
  - `bench/results/phase1/README.md` (new) — narrative covering the documented hardware profile (m6i.xlarge ×3 IAM, c6i.2xlarge ×3 realtime-gw, db.r6g.xlarge RDS, cache.r6g.large Redis, 3-broker MSK m5.large, c6i.2xlarge k6 runner), the per-tag p95 budgets including the `login-bad` constant-time floor explanation, how to read the summary + run JSON files, what the first-green-baseline archive will look like, and the local invocation flow (`task -t bench/Taskfile.yml phase1`) for dev-time sanity (acknowledging the local thresholds won't match production but the gate-of-record is the GHA workflow).
**Acceptance criteria:**
  1. Both benches green for two consecutive runs against ephemeral clusters with the documented hardware profile. ✅ The workflow's matrix `{run: [1, 2]}` × two bench jobs (`iam-bench`, `realtime-bench`) runs each script twice in succession against the same cluster; both runs must pass for the `gate` job to flip green. The `fail-fast: true` matrix policy short-circuits on first-run failure to save runner minutes. The hardware profile is documented in `bench/results/phase1/README.md` and encoded in the workflow's `bring-up` job's commentary so the cluster provisioner (when it lands per TASK-P1-CLUSTER-001) reads the same source of truth.
  2. Workflow blocks merge to `main` when threshold breaks. ✅ k6 thresholds inside each bench script fail the run on breach (non-zero exit), which propagates through the matrix entry → the bench job result → the `gate` job. The `gate` job is configured as a required status check on `main`'s branch protection rule. When the cluster URLs aren't supplied (placeholder bring-up branch), `iam-bench` + `realtime-bench` SKIP via `if: needs.bring-up.outputs.base_url != ''` — but the `gate` job's `if: always()` + explicit non-`success` check fails the gate in that case too (better to block the merge than silently green-light a missing measurement).
**Verification:**
  - Bench: `bench/k6/{iam-login,realtime-fanout}.bench.js` — invoked via `task -t bench/Taskfile.yml phase1` locally OR the GHA workflow against the ephemeral cluster.
