// Command sat-command is the satellite command-catalog and uplink-queue
// service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/sat-command/internal/config"
	"github.com/ppusapati/space/services/sat-command/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("sat-command: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewCommandHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := satcommandv1connect.NewCommandServiceHandler(h, connectOption)
