// Command gs-rf is the ground-station RF link service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gs-rf/internal/config"
	"github.com/ppusapati/space/services/gs-rf/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gs-rf: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewRFHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gsrfv1connect.NewRFServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
