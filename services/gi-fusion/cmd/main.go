// Command gi-fusion is the multi-source data fusion service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gi-fusion/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gi-fusion: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewFusionHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gifusionv1connect.NewFusionServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
