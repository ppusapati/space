// Package config loads sat-telemetry configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("sat-telemetry: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("sat-telemetry: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "sat-telemetry"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("sat-telemetry: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
