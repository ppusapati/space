// Command gi-reports is the GEOINT report-generation service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gi-reports/internal/config"
	"github.com/ppusapati/space/services/gi-reports/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gi-reports: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewReportsHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gireportsv1connect.NewReportsServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
