// Package config loads gi-reports configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("gi-reports: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("gi-reports: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "gi-reports"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("gi-reports: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
