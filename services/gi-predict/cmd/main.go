// Command gi-predict is the GEOINT forecast service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gi-predict/internal/config"
	"github.com/ppusapati/space/services/gi-predict/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gi-predict: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewPredictHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gipredictv1connect.NewForecastServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
