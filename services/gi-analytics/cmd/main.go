// Command gi-analytics is the GEOINT analysis-job service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gi-analytics/internal/config"
	"github.com/ppusapati/space/services/gi-analytics/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gi-analytics: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewAnalyticsHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gianalyticsv1connect.NewAnalyticsServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
