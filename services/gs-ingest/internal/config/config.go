// Package config loads gs-ingest configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("gs-ingest: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("gs-ingest: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "gs-ingest"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("gs-ingest: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
