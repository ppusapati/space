// Command gs-ingest is the ground-station downlink ingest service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gs-ingest/internal/config"
	"github.com/ppusapati/space/services/gs-ingest/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gs-ingest: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewIngestHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gsingestv1connect.NewIngestServiceHandler(h, pkgserver.NewConnectOption(mwCfg))
