// Package config loads sat-fsw configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("sat-fsw: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("sat-fsw: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "sat-fsw"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("sat-fsw: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
