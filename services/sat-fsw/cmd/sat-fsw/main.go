// Command sat-fsw is the flight software registry service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/sat-fsw/internal/config"
	"github.com/ppusapati/space/services/sat-fsw/internal/handlers"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("sat-fsw: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	handler := handlers.NewFSWHandler(svc, []byte(cfg.CursorSecret))

	mux := http.NewServeMux()
	path, h := satsubsysv1connect.NewFlightSoftwareServiceHandler(
		handler,
