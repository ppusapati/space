...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("gs-mc: load %s: %w", path, err)
	}
	defer c.Close()

	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("gs-mc: scan config: %w", err)
	}
	applyDefaults(&cfg)
	overlayEnv(&cfg)
...
func validate(c *Config) error {
	if c.Database.DSN == "" {
		return errors.New("gs-mc: database.dsn (or DATABASE_URL) is required")
	}
	return nil
}
