// Command gs-mc is the ground-station mission-control catalog service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/gs-mc/internal/config"
	"github.com/ppusapati/space/services/gs-mc/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("gs-mc: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewMCHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := gsmcv1connect.NewMissionControlServiceHandler(h, connectOption)
