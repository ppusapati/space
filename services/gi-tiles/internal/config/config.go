// Package config loads gi-tiles configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("gi-tiles: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("gi-tiles: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "gi-tiles"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("gi-tiles: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
