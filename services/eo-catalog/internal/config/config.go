// Package config loads eo-catalog configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("eo-catalog: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("eo-catalog: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "eo-catalog"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("eo-catalog: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
