// Package config loads eo-pipeline configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("eo-pipeline: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("eo-pipeline: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "eo-pipeline"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("eo-pipeline: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
