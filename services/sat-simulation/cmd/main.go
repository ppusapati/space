// Command sat-simulation is the SITL/HITL simulation orchestration service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/sat-simulation/internal/config"
	"github.com/ppusapati/space/services/sat-simulation/internal/handler"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("sat-simulation: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	h, err := handler.NewSimulationHandler(svc)
	if err != nil {
		return err
	}
	...
	path, hndl := satsimulationv1connect.NewSimulationServiceHandler(h, connectOption)
