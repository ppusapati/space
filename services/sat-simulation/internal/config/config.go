...
// Load reads the YAML config and overlays env vars.
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("sat-simulation: load %s: %w", path, err)
	}
	defer c.Close()

	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("sat-simulation: scan config: %w", err)
	}
	applyDefaults(&cfg)
	overlayEnv(&cfg)
...
func validate(c *Config) error {
	if c.Database.DSN == "" {
		return errors.New("sat-simulation: database.dsn (or DATABASE_URL) is required")
	}
	return nil
}

// HTTPPort returns the port suffix from HTTP.Addr.
