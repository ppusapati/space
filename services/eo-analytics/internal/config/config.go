// Package config loads eo-analytics configuration via packages/config.
package config
...
func Load() (Config, error) {
	...
	if err := c.Load(); err != nil {
		return Config{}, fmt.Errorf("eo-analytics: load %s: %w", path, err)
	}
	defer c.Close()
	var cfg Config
	if err := c.Scan(&cfg); err != nil {
		return Config{}, fmt.Errorf("eo-analytics: scan: %w", err)
	}
	if cfg.Service.Name == "" {
		cfg.Service.Name = "eo-analytics"
	}
	...
	if cfg.Database.DSN == "" {
		return Config{}, errors.New("eo-analytics: database.dsn (or DATABASE_URL) is required")
	}
	return cfg, nil
}
