// Command eo-analytics is the model registry + ML inference orchestration service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/eo-analytics/internal/config"
	"github.com/ppusapati/space/services/eo-analytics/internal/handlers"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("eo-analytics: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	logger.Info("starting eo-analytics",
	...
	handler := handlers.NewAnalyticsHandler(svc, []byte(cfg.CursorSecret))

	mux := http.NewServeMux()
	path, h := earthobsv1connect.NewAnalyticsServiceHandler(
		handler,
