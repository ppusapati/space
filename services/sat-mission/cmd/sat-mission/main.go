// Command sat-mission is the satellite mission registry service.
package main
...
import (
	...
	"github.com/ppusapati/space/services/sat-mission/internal/config"
	"github.com/ppusapati/space/services/sat-mission/internal/handlers"
...
func main() {
	if err := run(); err != nil {
		_, _ = os.Stderr.Write([]byte("sat-mission: " + err.Error() + "\n"))
		os.Exit(1)
	}
}
...
func run() error {
	...
	logger.Info("starting sat-mission",
	...
	repo := repository.NewSatelliteRepository(pool)
	svc := service.New(repo)
	handler := handlers.NewMissionHandler(svc, []byte(cfg.CursorSecret))

	mux := http.NewServeMux()
	path, h := satsubsysv1connect.NewMissionServiceHandler(
		handler,
