...
import { DEFAULT_COLOR_SCALE, DEFAULT_NEUTRAL, DEFAULT_TYPOGRAPHY, DEFAULT_SPACING, DEFAULT_BORDER_RADIUS, DEFAULT_SHADOWS, } from './theme.types.js';
// =============================================================================
// THEME PRESETS
// =============================================================================
const THEME_PRESETS = [
...
// DEFAULT THEME
// =============================================================================
function createDefaultTheme(mode = 'light') {
    ...
    return {
        ...
        initialize: () => {
            update(state => {
                ...
                const savedMode = typeof localStorage !== 'undefined'
                    ? localStorage.getItem('samavāya-theme-mode')
                    : null;
                const mode = savedMode || (state.mode === 'system' ? getSystemMode() : state.mode);
                const themeId = savedThemeId || 'default';
                const theme = state.themes.find((t) => t.id === themeId && t.mode === mode)
                    || state.themes.find((t) => t.mode === mode)
                    || createDefaultTheme(mode);
                applyTheme(theme);
                return {
                    ...state,
        ...
        setMode: (mode) => {
            update(state => {
                ...
                const theme = state.themes.find((t) => t.id === state.currentThemeId && t.mode === effectiveMode)
                    || state.themes.find((t) => t.mode === effectiveMode)
        ...
        toggleMode: () => {
            update(state => {
                ...
                const theme = state.themes.find((t) => t.id === state.currentThemeId && t.mode === newMode)
                    || state.themes.find((t) => t.mode === newMode)
        ...
        applyPreset: (presetId) => {
            update(state => {
                ...
                if (!preset)
                    return state;
                const baseTheme = createDefaultTheme(state.mode === 'system' ? getSystemMode() : state.mode);
        ...
        deleteTheme: (themeId) => {
            update(state => {
                ...
                if (state.currentThemeId === themeId) {
                    const defaultTheme = themes.find((t) => t.id === 'default' && t.mode === state.mode)
                        || themes[0];
                    if (defaultTheme)
...
export const currentTheme = derived(themeStore, ($store) => {
    const effectiveMode = $store.mode === 'system'
        ? (typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')
        : $store.mode;
    return $store.themes.find((t) => t.id === $store.currentThemeId && t.mode === effectiveMode)
        || $store.themes.find((t) => t.mode === effectiveMode)
        || createDefaultTheme(effectiveMode);
});
export const themeMode = derived(themeStore, ($store) => $store.mode);
export const themePresets = derived(themeStore, ($store) => $store.presets);
export const customThemes = derived(themeStore, ($store) => $store.themes.filter((t) => !t.isSystem));
