...
import { Size, ValidationState } from './common.types.js';
/**
 * Generic Form Type
 * @template TValues - Form values object type
 * @template TValidation - Validation schema type
 */
export interface Form<TValues extends Record<string, unknown> = Record<string, unknown>, TValidation extends FormValidation<TValues> = FormValidation<TValues>> {
    values: TValues;
...
/** Validate function */
export type ValidateFn<TValue> = (value: TValue, formValues?: Record<string, unknown>) => boolean | string | Promise<boolean | string>;
/** Validation rule */
...
/** Date range field */
export interface DateRangeField extends FormField<{
    start: Date | null;
    end: Date | null;
}> {
    type: 'daterange';
...
export type RichTextToolbarItem = 'bold' | 'italic' | 'underline' | 'strike' | 'heading' | 'blockquote' | 'code' | 'link' | 'image' | 'video' | 'list' | 'bullet' | 'indent' | 'align' | 'color' | 'background' | 'clean';
/** File upload field */
...
/** Autocomplete field */
export interface AutocompleteField<TOption = unknown> extends FormField<TOption | TOption[] | null> {
    type: 'autocomplete';
    options?: TOption[];
...
export type FormFieldConfig = TextField | NumberField | SelectField | DateField | DateRangeField | CheckboxField | CheckboxGroupField | RadioField | SwitchField | TextareaField | RichTextField | FileField | AutocompleteField | ColorField | SliderField | RatingField | ArrayField | ObjectField | HiddenField | CustomField;
/** Form schema */
