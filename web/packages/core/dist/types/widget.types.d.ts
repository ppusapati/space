...
import { BaseError, ColorVariant, Size } from './common.types.js';
/**
 * Generic Widget Type
 * @template TData - Widget data type
 * @template TConfig - Widget configuration type
 */
export interface Widget<TData = unknown, TConfig extends WidgetConfig = WidgetConfig> {
    id: string;
    type: WidgetType;
    data: TData | null;
    config: TConfig;
...
/** Widget type */
export type WidgetType = 'metric' | 'chart' | 'table' | 'list' | 'calendar' | 'timeline' | 'map' | 'progress' | 'activity' | 'custom';
...
/** Chart type */
export type ChartType = 'line' | 'bar' | 'pie' | 'doughnut' | 'area' | 'scatter' | 'radar' | 'polarArea' | 'bubble';
/** Chart widget config */
 ...
 */
export interface TableWidget<TRow = Record<string, unknown>> extends Widget<TRow[], TableWidgetConfig<TRow>> {
    type: 'table';
    pagination?: {
        page: number;
        pageSize: number;
 ...
 */
export interface ListWidget<TItem = Record<string, unknown>> extends Widget<TItem[], ListWidgetConfig<TItem>> {
    type: 'list';
}
/** List widget config */
 ...
 */
export interface CalendarWidget<TEvent extends CalendarWidgetEvent> extends Widget<TEvent[], CalendarWidgetConfig> {
    type: 'calendar';
    currentDate: Date;
 ...
 */
export interface TimelineWidget<TItem extends TimelineWidgetItem> extends Widget<TItem[], TimelineWidgetConfig> {
    type: 'timeline';
}
 ...
 */
export interface MapWidget<TMarker extends MapMarker> extends Widget<TMarker[], MapWidgetConfig> {
    type: 'map';
    center: {
        lat: number;
        lng: number;
 ...
 */
export interface ActivityWidget<TActivity extends ActivityWidgetItem> extends Widget<TActivity[], ActivityWidgetConfig> {
    type: 'activity';
}
