 ...
 */
export interface Layout<TSlots extends string = 'default', TConfig extends LayoutConfig = LayoutConfig> {
    config: TConfig;
 ...
 */
export interface MasterDetailLayout<TMaster = unknown, TDetail = unknown> extends Layout<'master' | 'detail', MasterDetailConfig> {
    masterData: TMaster[];
 ...
 */
export interface DashboardLayout<TWidget extends string = string> extends Layout<'widgets' | 'sidebar' | 'header', DashboardLayoutConfig> {
    widgets: WidgetLayout<TWidget>[];
    gridCols: number;
    gridRows: number;
    onWidgetResize: (widgetId: TWidget, size: {
        cols: number;
        rows: number;
    }) => void;
    onWidgetMove: (widgetId: TWidget, position: {
        col: number;
        row: number;
    }) => void;
 ...
 */
export interface TabLayout<TTab extends string = string> extends Layout<TTab, TabLayoutConfig> {
    activeTab: TTab;
    tabs: TabConfig<TTab>[];
 ...
 */
export interface WizardLayout<TStep extends string = string> extends Layout<TStep, WizardLayoutConfig> {
    currentStep: TStep;
 ...
 */
export interface AppShellLayout extends Layout<'header' | 'sidebar' | 'main' | 'footer', AppShellConfig> {
