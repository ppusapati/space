...
export async function uploadFileWithProgress(file, options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (auth.tokens?.accessToken) {
            xhr.setRequestHeader('Authorization', `Bearer ${auth.tokens.accessToken}`);
        }
        // Setup timeout
        if (options.timeout) {
 ...
 */
export async function uploadChunked(file, options = {}) {
    const config = getConfig();
    const auth = get(authStore);
    ...
    // Complete upload
    const completeResponse = await fetch(`${config.baseUrl}${UPLOAD_ENDPOINTS.complete}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            ...(auth.tokens?.accessToken && {
 ...
 */
async function uploadChunk(state, chunkIndex, chunk, options) {
    const config = getConfig();
    const auth = get(authStore);
    const formData = new FormData();
 ...
 */
export async function uploadMultiple(files, options = {}) {
