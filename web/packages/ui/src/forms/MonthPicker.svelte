<script lang="ts">
  ...
  export let helperText: string = 'Format: YYYY-MM';
  ...
  export let id: string = uid('month-picker');
  ...
  function selectMonth(month: number) {
    const selectedValue = `${displayYear}-${String(month + 1).padStart(2, '0')}`;
    value = selectedValue;
    isOpen = false;
  ...
  $: {
    if (value) {
      ...
      displayYear = parseInt(parts[0] ?? '0');
      displayMonth = parseInt(parts[1] ?? '1') - 1;
    }
  }
</script>
...
<div class={cn('w-full', className)}>
  ...
  <div class="relative">
    ...
    {#if isOpen}
      <div
        class="absolute z-10 w-full mt-1 bg-white border border-neutral-200 rounded-md shadow-lg p-4"
      >
        <!-- Year Selection -->
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="font-medium text-neutral-900">{displayYear}</span>
          <button
            type="button"
            on:click={nextYear}
            class="p-1 hover:bg-neutral-100 rounded"
        ...
        <div class="grid grid-cols-3 gap-2">
          {#each months as month, idx}
            <button
              type="button"
              on:click={() => selectMonth(idx)}
              class={cn(
                'py-2 px-2 rounded text-sm transition-colors',
                displayMonth === idx && value
